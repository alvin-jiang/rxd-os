...
all : $(ASMOUT) $(szfiles)
	...
	dd if=$(szfiles) of=a.img bs=1 seek=508 count=1 conv=notrunc
	dd if=boot/setup.bin of=a.img bs=512 seek=1 conv=notrunc count=`du --apparent-size --block-size=512 boot/setup.bin | awk '{printf $$1}'`
